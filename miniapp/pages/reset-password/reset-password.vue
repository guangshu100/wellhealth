<template>
  <view class="reset-container">
    <view class="header">
      ...
      <text class="subtitle">通过邮箱验证码重置</text>
    </view>
...
export default {
  ...
  methods: {
    async sendCode() {
      ...
      try {
        await emailCodeApi.sendCode(this.email, 'reset')
    ...
    async handleReset() {
      ...
      this.loading = true
      try {
        await emailCodeApi.resetPassword(this.email, this.code, this.newPassword)
        ...
        uni.showToast({
          title: err.message || err.detail || '重置失败',
          icon: 'none'
        })
      } finally {
        this.loading = false
